...
if(import.meta.env.VITE_STAGING_SANDBOX){
  ...
  function renderMyGlobe(records){
    ...
    myGlobePlacements.innerHTML=myGlobeRecords.length?myGlobeRecords.map(record=>`<article class="my-globe-card" data-placement-id="${escapeMyGlobe(record.placementId)}">${record.artworkDataUrl?`<img src="${escapeMyGlobe(record.artworkDataUrl)}" alt="">`:'<div class="my-globe-placeholder" aria-hidden="true">⬡</div>'}<div class="my-globe-card-copy"><div class="my-globe-card-heading"><h3>${escapeMyGlobe(record.title||'Untitled placement')}</h3><span>${escapeMyGlobe(record.publicationStatus==='published'?'Published':record.publicationStatus||record.status)}</span></div><p>${record.cellCount.toLocaleString()} ${record.cellCount===1?'cell':'cells'} · Version ${record.currentVersion||1}</p><small>Claimed ${record.createdAt?new Date(record.createdAt).toLocaleDateString(undefined,{day:'numeric',month:'short',year:'numeric'}):'recently'}</small><div class="my-globe-actions"><button type="button" data-owner-action="view">View on globe</button><button type="button" data-owner-action="edit">Edit details</button></div><form class="my-globe-edit" hidden><label>Name<input name="title" maxlength="120" required value="${escapeMyGlobe(record.title||'Untitled placement')}"></label><label>Description<textarea name="description" maxlength="500" rows="3">${escapeMyGlobe(record.description||'')}</textarea></label><label>Website<input name="destinationUrl" type="url" value="${escapeMyGlobe(record.destinationUrl||'')}"></label><div><button type="button" data-owner-action="cancel">Cancel</button><button type="submit">Publish update</button></div><small role="status"></small></form></div></article>`).join(''):'<div class="my-globe-empty"><b>Your globe is waiting.</b><p>Purchases made with this verified email will appear here automatically.</p></div>';
  }
  async function openMyGlobe(){
    accountPanel.hidden=true;accountToggle.setAttribute('aria-expanded','false');myGlobe.hidden=false;document.body.classList.add('my-globe-open');myGlobeStatus.textContent='Loading your placements…';myGlobePlacements.replaceChildren();
    try{const records=await stagingClient.listTestClaims();renderMyGlobe(records);myGlobeStatus.textContent='';}
    catch(error){myGlobeStatus.textContent=`Could not load your placements: ${error.message}`;}
  }
  ...
  myGlobePlacements.onclick=event=>{const button=event.target.closest('[data-owner-action]');if(!button)return;const card=button.closest('.my-globe-card'),record=myGlobeRecords.find(item=>item.placementId===card.dataset.placementId);if(!record)return;if(button.dataset.ownerAction==='view'){closeMyGlobe();initialPlacementFocusAllowed=true;flyToCell(record.anchor,.004,900,()=>inspectPlacement(record.anchor));}else if(button.dataset.ownerAction==='edit'){card.querySelector('.my-globe-edit').hidden=false;button.closest('.my-globe-actions').hidden=true;card.querySelector('input').focus();}else if(button.dataset.ownerAction==='cancel'){card.querySelector('.my-globe-edit').hidden=true;card.querySelector('.my-globe-actions').hidden=false;}};
  myGlobePlacements.onsubmit=async event=>{event.preventDefault();const form=event.target,card=form.closest('.my-globe-card'),button=form.querySelector('[type=submit]'),formStatus=form.querySelector('[role=status]'),content=Object.fromEntries(new FormData(form));button.disabled=true;formStatus.textContent='Publishing your update…';try{await stagingClient.updatePlacementMetadata(card.dataset.placementId,content);formStatus.textContent='Update queued for publication.';await openMyGlobe();}catch(error){formStatus.textContent=`Could not publish: ${error.message}`;}finally{button.disabled=false;}};
