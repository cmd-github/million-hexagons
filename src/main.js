...
async function openShareCard(source,{prompt=false}={}){const record={...source,name:source.name||source.title,count:source.count||source.cellCount,logo:source.logo||source.artworkDataUrl||source.thumbnailDataUrl};if(!record?.placementId||record.publicationStatus&&record.publicationStatus!=='published')return false;const promptKey=`mh-share-prompted:${record.placementId}`;if(prompt&&localStorage.getItem(promptKey))return false;if(prompt)localStorage.setItem(promptKey,'1');const url=placementShareUrl(record,record.anchor),copy=cardCopy(record,document.querySelector('#shareCardFormat').value);activeShare={record,url};document.querySelector('#shareCardName').textContent=copy.headline;document.querySelector('#shareCardDescription').textContent=copy.support;document.querySelector('#shareCardCells').textContent=record.count.toLocaleString();document.querySelector('#shareCardLocation').textContent=copy.hexId;document.querySelector('#shareCardUrl').textContent=url;document.querySelector('#shareCardStatus').textContent='';document.querySelector('#shareCard').hidden=false;document.body.classList.add('sharing-placement');await refreshSharePreview();document.querySelector('#nativeSharePlacement').focus();return true;}
...
document.querySelector('#downloadSharePlacement').onclick=async event=>{if(!activeShare)return;event.currentTarget.disabled=true;document.querySelector('#shareCardStatus').textContent='Preparing download…';try{await downloadShareCard(activeShare.record,document.querySelector('#shareCardFormat').value);document.querySelector('#shareCardStatus').textContent='Downloaded ✓';trackEvent('placement_shared',{placementId:activeShare.record.placementId,context:{source:'share'},unique:true});}finally{event.currentTarget.disabled=false;}};
...
document.querySelector('#nativeSharePlacement').onclick=async()=>{const copy=cardCopy(activeShare.record,document.querySelector('#shareCardFormat').value),data={title:`${copy.name} on Million Hexagons`,text:copy.shareText,url:activeShare.url};if(navigator.share)try{await navigator.share(data);trackEvent('placement_shared',{placementId:activeShare.record.placementId,context:{source:'share'},unique:true});return;}catch(error){if(error.name==='AbortError')return;}document.querySelector('#copySharePlacement').click();};
