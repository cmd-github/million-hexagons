...
Available inventory is plated cell-by-cell in a restrained palette of near-black, ink navy, and occasional muted cobalt, with stable shades derived from each logical hex address. The variation resolves only when cells are large enough to perceive, preserving a calm midnight-blue globe at distance. A restrained camera-fixed highlight, gently lifted south pole, and surface-attached Fresnel edge complete the treatment. Available cells are restored after artwork sampling so neighbouring advertiser colours cannot bleed across territory boundaries; advertiser artwork and placement-state overlays are unchanged.
