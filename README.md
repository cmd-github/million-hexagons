...
- A shared right-hand studio carries Design and Review with consistent navigation and controls. A fresh placement first keeps the globe movable while the visitor zooms to visible hexagons, selects an available starting spot and explicitly confirms it with Start here. Design then contains Draw, Delete, Move globe and Choose another spot; confirming another spot moves the complete design intact. Drawing on an available adjacent hexagon adds it directly. Escape unwinds one layer at a time — open menu, then checkout, then the studio — and closing the studio keeps the in-progress design, so reopening it from Create a placement restores the work. Opening for an owner update starts clean; Start over in the design menu discards a restored draft.
- Compact globe-only editor for 1-100,000 hexagons: large exact-count minus/plus controls, background colour, recently used colour swatches, four hexagon-shaped brushes, direct drawing/growth, connected deletion, image movement/zoom/rotation and undo/redo. Fresh placements start clean; owner editing preserves its locked purchased footprint.
...
Startup keeps the branded loader visible until the visible artwork preview is ready, then continues sharpening in the background. Slow or failed loads offer status and retry; subsequent artwork loading does not block exploration.
