...
The purchase flow separates navigation from selection: Move globe keeps rotation enabled, while Select hexagons locks rotation and supports click-and-drag painting for custom territories. A one-million-cell GPU occupancy mask gives available, purchased and selected tiles distinct states regardless of advertiser colour, and pointer selection uses the same nearest-hex calculation as the rendered grid. Selected cells use the current brand colour with a bright perimeter, so colour edits update directly on the globe.
