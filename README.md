...
The plane control starts an opt-in globe **Tour** generated from current placement metadata. Each batch selects up to 24 geographically varied purchased areas, targets the placement's central focus cell rather than a potentially blank edge, and mixes wide transitions with selected close-detail passes. A fresh batch is generated whenever the previous one finishes, so the tour continues until stopped. With no occupied inventory it stays at overview distance. Stop, Escape, manual globe interaction, other controls or resizing returns control to the visitor. Selection uses compact placement metadata and reuses the lazy topology rather than scanning inventory or adding per-frame work.
