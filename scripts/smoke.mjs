...
await page.locator('#brandColor').fill('#3366ff');
await page.mouse.click(690, 450);
const customStatus = await page.locator('#selectionStatus').textContent();
if (!customStatus.includes('2 colours')) errors.push(`Multi-colour painting status was: ${customStatus}`);
await page.click('#previewPurchase');
await page.waitForTimeout(300);
await page.click('#claimButton');
await page.click('#selectHexMode');
await page.mouse.click(690, 450);
const repurchaseStatus = await page.locator('#selectionStatus').textContent();
if (!repurchaseStatus.toLowerCase().includes('purchased')) errors.push(`Re-purchase guard status was: ${repurchaseStatus}`);
await page.click('#closeBuy');
console.log(JSON.stringify({ panel: await page.locator('#buyPanel').getAttribute('aria-hidden'), sold: await page.locator('#soldCount').textContent(), toast: await page.locator('#toast').isVisible(), uploadAccepted: await page.locator('#logoPreview img').count() === 1, logoRotation, customPrice, customStatus, repurchaseStatus, detectedColours: await page.locator('#logoSwatches button').count(), errors }));
await browser.close();
