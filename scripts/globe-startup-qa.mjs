...
try{for(const mobile of [false,true]){
 ...
 await page.goto(`http://127.0.0.1:${server.httpServer.address().port}/`,{waitUntil:'commit'});await page.waitForSelector('#world[data-ready=true]');assert.equal(await page.locator('#appLoading').isVisible(),false);
 await page.locator('#claimButton').click();await page.locator('#appLoading').waitFor({state:'visible'});await page.waitForTimeout(500);assert.equal(await page.locator('#designStep').isVisible(),false);
 release();await page.locator('#appLoading').waitFor({state:'hidden',timeout:90000});await page.locator('#designStep').waitFor({state:'visible'});await page.waitForTimeout(1600);assert.equal(await page.locator('#placementInspector').isVisible(),false,'Background restoration must not steal focus from editing');assert.equal(await page.evaluate(()=>performanceQA.state().inventoryLoaded),true);assert.deepEqual(errors,[]);console.log(JSON.stringify({mobile,globeBeforeInventory:true,editorWaitsForInventory:true,noFocusSteal:true}));await page.close();
