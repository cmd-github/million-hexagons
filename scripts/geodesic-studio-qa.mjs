...
try {
 for(const mobile of [false,true]) for(const type of ['logo','colour','paint']) {
  ...
  if(type==='logo') {
    await page.locator('#logoUpload').setInputFiles('scripts/fixtures/geodesic-reference.svg');await page.waitForFunction(()=>document.querySelector('#addImageLabel').textContent==='Replace image'&&document.querySelector('#uploadStatus').hidden);
