...
try{for(const mobile of [false,true]){
 ...
 const anchor=Number((await p.locator('#inspectorHex').textContent()).replace(/[^0-9]/g,''));
 ...
 assert.notEqual(await p.locator('#inspectorHex').textContent(),'#'+anchor.toLocaleString('en-US'));assert.deepEqual(await p.evaluate(()=>window.hudChanges),[]);await p.evaluate(()=>window.hudObserver.disconnect());
