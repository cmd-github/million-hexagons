...
try{for(const mobile of [false,true]){
 ...
 assert.match(await p.locator('#inspectorDate').textContent(),/\d{2}\/\d{2}\/\d{2}/);
 ...
 await p.locator('#inspectorVisit').dispatchEvent('auxclick',{button:1});assert.match(await p.locator('#inspectorClicks').textContent(),/^329$/);
 await p.reload();await p.waitForFunction(()=>window.geodesicQA);await p.locator('#toggleHexSearch').click();await p.locator('#hexSearchInput').fill('Nike');await p.locator('#companyResults button').first().click();assert.equal(await p.locator('#inspectorClicks').textContent(),'329');
