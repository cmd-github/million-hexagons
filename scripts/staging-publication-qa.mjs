...
try{
  ...
  const analyticsSession=crypto.randomUUID(),firstView=await request({action:'record-event',event:{placementId:created.placementId,type:'view',sessionId:analyticsSession}}),duplicateView=await request({action:'record-event',event:{placementId:created.placementId,type:'view',sessionId:analyticsSession}}),click=await request({action:'record-event',event:{placementId:created.placementId,type:'click',sessionId:`${analyticsSession}-click`}}),publicDetail=await request({action:'public-placement',placementId:created.placementId});
  assert.ok(firstView.response.ok);assert.equal(duplicateView.result.duplicate,true);assert.equal(click.result.metrics.clicks,1);assert.equal(publicDetail.result.placement.metrics.views,1);assert.equal(publicDetail.result.placement.metrics.clicks,1);assert.equal(publicDetail.result.placement.placementId,created.placementId);
