...
try{
  ...
  assert.ok(firstView.response.ok);assert.equal(duplicateView.result.duplicate,true);assert.equal(click.result.metrics.clicks,1);assert.equal(publicDetail.result.placement.metrics.views,1);assert.equal(publicDetail.result.placement.metrics.clicks,1);assert.equal(publicDetail.result.placement.placementId,created.placementId);assert.ok(funnel.response.ok);assert.ok(publicStats.result.stats.claimedCells>=created.cellCount);assert.ok(publicStats.result.stats.placements>=1);assert.ok(publicStats.result.latest.some(placement=>placement.placementId===created.placementId));
