...
try{
  ...
  const analyticsSession=crypto.randomUUID(),firstView=await request({action:'record-event',event:{placementId:created.placementId,type:'placement_viewed',sessionId:analyticsSession}}),duplicateView=await request({action:'record-event',event:{placementId:created.placementId,type:'placement_viewed',sessionId:analyticsSession}}),click=await request({action:'record-event',event:{placementId:created.placementId,type:'outbound_link_clicked',sessionId:`${analyticsSession}-click`}}),funnel=await request({action:'record-event',event:{type:'checkout_started',sessionId:analyticsSession,context:{cellCount:created.cellCount,deviceClass:'desktop',source:'checkout'}}}),publicDetail=await request({action:'public-placement',placementId:created.placementId}),publicStats=await request({action:'public-stats'});
  assert.ok(firstView.response.ok);assert.equal(duplicateView.result.duplicate,true);assert.equal(click.result.metrics.clicks,1);assert.equal(publicDetail.result.placement.metrics.views,1);assert.equal(publicDetail.result.placement.metrics.clicks,1);assert.equal(publicDetail.result.placement.placementId,created.placementId);assert.match(publicDetail.result.placement.openGraphImageUrl,/\/og\.webp$/);const openGraphBytes=Buffer.from(await (await fetch(publicDetail.result.placement.openGraphImageUrl)).arrayBuffer()),openGraphMetadata=await sharp(openGraphBytes).metadata();assert.equal(openGraphMetadata.width,1200);assert.equal(openGraphMetadata.height,630);const placementUrl=`https://million-hexagons-staging.million-hexagons.workers.dev/placement/${created.placementId}`,placementPage=await fetch(placementUrl,{headers:{'user-agent':'facebookexternalhit/1.1'}}),placementHtml=await placementPage.text();assert.ok(placementPage.ok);assert.match(placementHtml,new RegExp(`<link rel="canonical" href="${placementUrl}"`));assert.match(placementHtml,new RegExp(`<meta property="og:image" content="${publicDetail.result.placement.openGraphImageUrl.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}"`));assert.ok(funnel.response.ok);assert.ok(publicStats.result.stats.claimedCells>=created.cellCount);assert.ok(publicStats.result.stats.placements>=1);assert.ok(publicStats.result.latest.some(placement=>placement.placementId===created.placementId));
  ...
  const removeDescription=await request({action:'moderate',placementId:created.placementId,command:{action:'remove-description',reason:'Automated description test'}});assert.ok(removeDescription.response.ok);moderated=(await request({action:'public-list'})).result.placements.find(item=>item.placementId===created.placementId);assert.equal(moderated.description,'');
  const removeArtwork=await request({action:'moderate',placementId:created.placementId,command:{action:'remove-artwork',reason:'Automated artwork test'}});assert.ok(removeArtwork.response.ok);moderated=(await request({action:'public-list'})).result.placements.find(item=>item.placementId===created.placementId);assert.equal(moderated.artworkDataUrl,'');assert.equal(moderated.moderationStatus,'artwork-hidden');
  const suspended=await request({action:'moderate',placementId:created.placementId,command:{action:'suspend',reason:'Automated suspension test'}});assert.ok(suspended.response.ok);moderated=(await request({action:'public-list'})).result.placements.find(item=>item.placementId===created.placementId);assert.equal(moderated.title,'Claimed placement');assert.equal(moderated.description,'Content currently unavailable');
  const restored=await request({action:'moderate',placementId:created.placementId,command:{action:'restore-version',version:1,reason:'Automated rollback test'}});assert.ok(restored.response.ok);moderated=(await request({action:'public-list'})).result.placements.find(item=>item.placementId===created.placementId);assert.equal(moderated.title,'Automated publication QA');assert.match(moderated.artworkDataUrl,/\/versions\/1\/inspection\.webp$/);
