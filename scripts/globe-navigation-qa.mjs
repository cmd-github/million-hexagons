...
for(const [width,height] of [[320,568],[390,844],[1024,768],[1440,900]]){
 const p=await b.newPage({viewport:{width,height}});await p.goto('http://127.0.0.1:4180/?geodesicQA');await p.waitForSelector('#world[data-ready=true]',{timeout:60000});await p.locator('#claimButton').click();await p.locator('#designStep').waitFor({state:'visible'});
...
await p.goto('http://127.0.0.1:4180/?geodesicQA');await p.waitForSelector('#world[data-ready=true]',{timeout:60000});await p.locator('#claimButton').click();await p.locator('#designStep').waitFor({state:'visible'});
const id=await p.evaluate(()=>window.geodesicQA.state().design[0]);await p.locator('#closeBuy').click();
await p.evaluate(id=>window.geodesicQA.focus(id,.5),id);await p.waitForTimeout(300);
...
await p.goto('http://127.0.0.1:4180/?geodesicQA#cell='+id);await p.waitForSelector('#world[data-ready=true]',{timeout:60000});await p.waitForTimeout(3000);assert.ok(Math.hypot(...await p.evaluate(()=>window.geodesicQA.state().camera))<4.5);
