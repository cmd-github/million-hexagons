...
try{for(const mobile of [false,true]){
 ...
 await page.locator('#claimButton').click();await page.locator('#locationStep').waitFor({state:'visible'});const start=await page.evaluate(()=>window.geodesicQA.state().designAnchor);await page.evaluate(id=>window.geodesicQA.focus(id,.6),start);await page.waitForFunction(()=>window.geodesicQA.state().detailVertices>0);await page.waitForTimeout(250);const point=await page.evaluate(id=>window.geodesicQA.screen(id),start);if(mobile)await page.touchscreen.tap(point.x,point.y);else await page.mouse.click(point.x,point.y);await page.locator('#claimCell').waitFor({state:'visible'});await page.locator('#claimCell').click();await page.locator('#hexAmount').fill('50');await page.waitForFunction(()=>window.geodesicQA.state().design.length===50);await page.locator('#toDesign').click();await page.locator('#designStep').waitFor({state:'visible'});
 await page.locator('#brushColor').fill('#112233');await page.locator('#brushColor').dispatchEvent('input');await page.locator('#logoUpload').setInputFiles({name:'small.png',mimeType:'image/png',buffer:await sharp({create:{width:32,height:32,channels:4,background:'#d7ff55'}}).png().toBuffer()});await page.locator('#artworkQuality').waitFor({state:'visible'});
 ...
 await page.locator('#closeBuy').click();await page.emulateMedia({reducedMotion:'reduce'});await page.locator('#homeView').click();assert.equal(await page.locator('#rotationToggle').getAttribute('aria-pressed'),'false');
 await page.close();
}assert.deepEqual(errors,[]);console.log('Rotated artwork retains all four corners; upload warning and reduced motion passed.');}finally{await browser.close();}
