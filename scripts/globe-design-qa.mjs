...
for(const mobile of [false,true]){
 ...
 const neighbour=await page.evaluate(id=>window.geodesicQA.neighbours(id)[0],initial[0]);await clickCell(neighbour);await page.waitForFunction(()=>window.geodesicQA.state().design.length>1);
 const ids=await page.evaluate(()=>window.geodesicQA.state().design);
 await clickCell(initial[0]);await page.locator('#undoPaint').click();
