...
if(!builtJavaScript.includes('stagingCheckout'))throw Error('Staging build omitted the persistent checkout client.');
...
const csp = `default-src 'self'; script-src 'self' https://js.stripe.com 'unsafe-hashes' 'sha256-${retryHash}'; worker-src 'self' blob:; connect-src 'self' ${origin} https://identitytoolkit.googleapis.com https://securetoken.googleapis.com https://europe-west1-million-hexagons.cloudfunctions.net https://api.stripe.com https://r.stripe.com; frame-src https://js.stripe.com https://hooks.stripe.com; img-src 'self' data: blob: ${origin} https://*.stripe.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; object-src 'none'; base-uri 'none'; frame-ancestors 'none'; form-action 'self'`;
