    ...
    try {
      ...
      if (action === 'admin-lookup') {
        ...
        const placements=await Promise.all(documents.slice(0,25).map(async document=>{
          ...
          const order=orders.docs[0]?.data();return{placementId:placement.placementId,ownerId:placement.ownerId,status:placement.status||'active',cellCount:Number(placement.cellCount||0),currentVersion,publicState:placement.publicState||{},review:moderationReviewState(placement),createdAt:placement.createdAt?.toMillis?.()||null,credits:Number(balance.data()?.available||0),payment:order?{orderId:order.orderId,status:order.status,paymentStatus:order.paymentStatus,paidAmountMinor:Number(order.paidAmountMinor||0),refundedAmountMinor:Number(order.refundedAmountMinor||0),currency:order.currency||''}:null,versions:versionRows,actions:actionRows};
        }));
        response.status(200).json({ok:true,result:{query,placements}});return;
      }
      if(action==='admin-review-queue'){
        if(identity.stagingAdmin!==true){response.status(403).json({ok:false,error:'administrator-required'});return;}
        const db=getFirestore(),snapshots=await db.collection('stagingPlacements').where('moderationReview.status','==','pending').limit(100).get(),nowMs=Date.now();
        const active=snapshots.docs.map(document=>document.data()).filter(placement=>!['deleted','revoked'].includes(placement.status)).sort(compareModerationQueue).slice(0,50);
        const versions=await Promise.all(active.map(placement=>db.collection('stagingPlacementVersions').doc(`${placement.placementId}-v${placement.currentVersion||1}`).get()));
        const placements=active.map((placement,index)=>({placementId:placement.placementId,ownerId:placement.ownerId,cellCount:Number(placement.cellCount||0),currentVersion:Number(placement.currentVersion||1),title:versions[index].data()?.title||placement.title||'Untitled placement',review:moderationReviewState(placement,nowMs)}));
        response.status(200).json({ok:true,result:{placements,pending:placements.length,overdue:placements.filter(item=>item.review.overdue).length,targetHours:12}});return;
      }
      ...
      if (action === 'revoke') {
        ...
        let credits=null;if(Number(request.body.creditAmount)>0)credits=await issueCredits(getFirestore(),{ownerId:snapshot.data().ownerId,amount:request.body.creditAmount,reason:`Placement revocation: ${reason}`,source:'moderation',actorId:identity.uid,idempotencyKey:`revoke-${placementId}`},FieldValue.serverTimestamp());
...
export const publishStagingPlacement = onDocumentCreated(
  ...
  async event => {
    ...
    try {
      ...
      const derivativeTools=await import('./artwork-derivatives.js'),derivatives=await derivativeTools.artworkDerivatives(bytes),openGraph=await derivativeTools.openGraphCard(bytes,version),derivativePrefix=objects.artworkKey.slice(0,objects.artworkKey.lastIndexOf('/'));
      for(const [name,Body] of [['inspection',derivatives.canonical],['overview',derivatives.overview],['thumbnail',derivatives.thumbnail],['og',openGraph]])await client.send(new PutObjectCommand({Bucket:bucket,Key:`${derivativePrefix}/${name}.webp`,Body,ContentType:'image/webp',CacheControl:'public,max-age=31536000,immutable'}));
