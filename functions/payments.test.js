...
test('builds an explicit Managed Payments session without unsupported payment method parameters',()=>{
  ...
  assert.deepEqual(params.managed_payments,{enabled:true});assert.equal(params.mode,'payment');assert.equal(params.ui_mode,'embedded_page');assert.equal(params.line_items[0].price_data.tax_behavior,'inclusive');assert.equal(params.line_items[0].price_data.product_data.tax_code,'txcd_10000000');assert.equal('payment_method_types' in params,false);
...
test('retains the existing card-only staging session until Managed Payments is enabled',()=>{
  ...
  assert.deepEqual(params.payment_method_types,['card']);assert.equal('managed_payments' in params,false);assert.equal(params.line_items[0].price_data.product_data.tax_code,undefined);
});
