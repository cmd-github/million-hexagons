...
- Available globe cells now use an almost-black midnight-blue material with a tightly focused cobalt highlight and narrow electric-blue Fresnel edge inspired by the supplied visual reference. Emissive lift is confined to the face-on hotspot so physical lighting does not wash the whole sphere cyan. The change is restricted to available inventory and lighting; sample/purchased artwork, selection colours, topology and the continent-free product direction are unchanged.
