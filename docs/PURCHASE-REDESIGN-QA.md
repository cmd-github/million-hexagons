...
- Available globe cells now use an almost-black midnight-blue material with a tightly focused, camera-fixed cobalt highlight and an electric-blue Fresnel light attached directly to the sphere rather than a separate atmosphere shell. It therefore remains joined to the visible surface at every zoom. A separate modest south-pole lift prevents the lower globe falling to black without making it as bright as the central hotspot. Available cells are restored after filtered artwork sampling, preventing campaign colours from bleeding into neighbouring inventory at close range. Sample/purchased artwork, selection colours, topology and the continent-free product direction are otherwise unchanged.
