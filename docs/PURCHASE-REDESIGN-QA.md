...
- The right-side globe toolbar now groups ambient play/pause, the inventory-driven Tour (plane), search, zoom, and home. A close-range globe click pauses ambient rotation and the play control restarts it.
- Tile hover cards omit globe coordinates. Available cells are labelled Available; occupied demo cells show the sample advertiser name and destination domain instead of the generic Purchased label.
