...
- Available inventory is now grouped geometrically into larger hexagonal macro-plates, each comprising neighbouring logical cells and receiving one deterministic near-black, ink-navy, or occasional muted-cobalt shade. This creates broad unified regions instead of cell-by-cell noise and fades at extreme distance to avoid shimmer. A restrained camera-fixed highlight, modest south-pole lift, and surface-attached Fresnel edge preserve the sphere treatment at every zoom. Available cells are restored after filtered artwork sampling, preventing campaign colours from bleeding into neighbouring inventory. Sample/purchased artwork, selection colours, topology and the continent-free product direction are otherwise unchanged.
