...
- Available globe cells now use an almost-black midnight-blue material with a tightly focused, camera-fixed cobalt highlight and a narrow but visible electric-blue Fresnel edge inspired by the supplied visual reference. A separate modest south-pole lift prevents the lower globe falling to black without making it as bright as the central hotspot. Emissive lift remains confined to the hotspot so physical lighting does not wash the whole sphere cyan. The change is restricted to available inventory and lighting; sample/purchased artwork, selection colours, topology and the continent-free product direction are unchanged.
