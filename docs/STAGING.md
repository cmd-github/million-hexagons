...
Latest frontend deployment (20 September 2026): `20f10eb4-ea65-4b9f-b69d-030126103f7d`. The studio stages fit without scrolling by having compact rows rather than a tall panel. Each image slider was stacking its label above the control, costing about 25px apiece; those are single rows now, and the layout choice, action buttons, size card, stepper, presets, brushes and summary all sit tighter, with the step words dropping on short screens. Panels are sized to the measured minimum for the tallest state, Design with an image loaded: 51% of the screen at 390x844 and 67% at 320x568, against 53% and 76% after the previous pass. `test:mobile-composition` measures with an image loaded and fails if a stage scrolls or a panel exceeds its share; that part needs the dev build and reports as skipped against a deployed origin.
