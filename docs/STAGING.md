...
Latest frontend deployment (14 September 2026): `e8cd408a-72cf-475b-8d7e-5f7ed68bdb13` from source commit `5e94f583`, referencing immutable runtime release `6121418c93f84b0a7fd8b572f3f386de309edec9ba6bc2aefcc2d44e084374b2`. Mobile browsers now scale the same desktop composition from an 800px layout viewport instead of 900px, making text and controls about 12.5% larger. The existing lime hexagon-star outlines are larger, thicker and more opaque without increasing their count. No mobile dock, bottom sheet, narrow camera branch or width/orientation media-query layout was reintroduced. Local desktop and 390x844/320x568 composition, globe interaction, startup, gestures and checkout-restoration checks passed; local and live screenshots were inspected. Live desktop/mobile Design -> Place -> Review -> Stripe mount/release/reload passed with zero browser errors and no payment submitted. Strict HTTPS/CDN health checks passed with cache HITs after all 1,549 runtime objects were verified. Physical iOS/Android devices remain unverified; the existing renderer remains enabled (`VITE_ARTWORK_SNAPSHOTS=false`).
