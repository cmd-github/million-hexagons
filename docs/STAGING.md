...
Latest frontend deployment (26 September 2026): `af9d3c09-ae67-4c1b-9677-21402d09a356` from source commit `786dea68` on branch `review-25-09-26`, retaining immutable runtime release `6121418c93f84b0a7fd8b572f3f386de309edec9ba6bc2aefcc2d44e084374b2`. It adds the 10,000-hexagon self-service purchase maximum and the large-request contact route (`millionhexagons@birdcagetech.com`). Cloud Functions `stagingPlacements`, `stagingCheckout` and `stripeWebhook` were deployed with it, so the cap holds at the server boundary too: live probes show 10,001 and 100,000 cells now rejected as `invalid-quote` where they previously reached the occupancy check, and the live publication journey still reserves, conflicts on and releases exactly 10,000. All 1,549 runtime objects, CORS, content types and cache policy verified. Roll back to `f5f9cb19-307c-4ea4-858a-4e28d8605ca2` to restore the previous front end.

Previous frontend deployment (26 September 2026): `f5f9cb19-307c-4ea4-858a-4e28d8605ca2`, retaining immutable runtime release `6121418c93f84b0a7fd8b572f3f386de309edec9ba6bc2aefcc2d44e084374b2`. It carries the 25 September review: the search panel opens level with its button, rotation is back in the mobile rail, a click no longer ends the globe tour, the sign-in link confirms with a drawn hexagon and tick, the hero pricing selector is gone, mobile placement details are a full-width band beneath the rail, the share card is one image plus X/LinkedIn/Facebook/download/copy, sharing a just-purchased placement re-checks publication instead of failing silently, editing a placement no longer shows the saved artwork underneath the draft, and account credits can be granted by email and spent against hexagons. Cloud Functions `stagingPlacements`, `stagingCheckout` and `stripeWebhook` were deployed first so the credit-aware checkout existed before the front end could call it; Firebase Hosting and the coming-soon site were not touched. All 1,549 runtime objects, CORS, content types and cache policy verified; the launch gate, 59 backend tests and the browser suites pass, and account-access, public-placement, my-globe and regional-pricing pass against this deployment. No card payment or credit redemption has been run against live Stripe on this deployment, and no physical device was used. Roll back to `0b808d6e-6eb1-4d61-92b0-4dfc43b4db96` to restore the previous front end.

Previous frontend deployment (20 September 2026): `0b808d6e-6eb1-4d61-92b0-4dfc43b4db96`. A physical Android Chrome screenshot showed what the emulator did not: a dead band of empty panel under the last studio control, and roughly half the globe the emulated runs reported. The panel was positioned `top:Xdvh` with `bottom:0`, and on Chrome Android a fixed element's bottom edge and a `dvh` percentage do not resolve against the same box while the browser toolbar is showing, so the panel came out taller than the percentage implied. It is now anchored to the bottom with an automatic height, capped with `svh`. Measured at 411x795: Shape and Design take 50% and 53% of the screen with 1px of dead space, against a reported 67% with a visible band; Location drops to 26%. `test:mobile-composition` measures at that viewport and fails on dead space below the content.

**Emulated viewports did not catch this.** Treat physical-device inspection as required evidence for phone layout, not as confirmation of an emulated result.

Headed Android emulator pass, 20 September 2026, `Pixel_8a_API_35`, Android 15 / API 35, Chrome 153, portrait `411x812` CSS at DPR 2.625, against this staging deployment. Browse: pitch sheet 131px (16% of the viewport), bottom-anchored; Claim in the header at y 10-48; controls lower right; activity ticker hidden until zoomed; canvas full-bleed. Studio, measured through a real Location -> Shape -> Design journey: Location panel 27%, Shape 50%, Design 51%, each with **0px scroll overflow and 1px dead space**, which is the `dvh` defect fixed and confirmed on a real Android Chrome rather than an emulated viewport. Rotation to landscape and back recovers cleanly, and Android Back steps Design -> Shape. Screenshots in `artifacts/android-emulator/`.

Open issues that pass found, neither caused by this work:

- **Landscape studio overlaps and clips.** At `868x327` CSS the phone breakpoint does not match (`matchMedia('(max-width:700px)')` is false), so the desktop side-panel layout runs in a 327px-tall viewport: the Image and Add image controls overlap and the step overflows by 31px. All of this work's studio rules live inside `@media(max-width:700px)`, so landscape is unchanged by it. Needs a short-viewport breakpoint.
- **First load on this emulator took about 80 seconds** to interactive under software WebGL, and the tab was discarded and fully reloaded after Chrome was backgrounded on its 2GB device. Not evidence about real-device performance, which has a GPU, but worth measuring properly.

Not covered by this pass: Design with an image loaded on the device, iOS/Safari, and any physical handset.

Previous frontend deployment (20 September 2026): `20f10eb4-ea65-4b9f-b69d-030126103f7d`. The studio stages fit without scrolling by having compact rows rather than a tall panel. Each image slider was stacking its label above the control, costing about 25px apiece; those are single rows now, and the layout choice, action buttons, size card, stepper, presets, brushes and summary all sit tighter, with the step words dropping on short screens. Panels are sized to the measured minimum for the tallest state, Design with an image loaded: 51% of the screen at 390x844 and 67% at 320x568, against 53% and 76% after the previous pass. `test:mobile-composition` measures with an image loaded and fails if a stage scrolls or a panel exceeds its share; that part needs the dev build and reports as skipped against a deployed origin.

Previous frontend deployment (20 September 2026): `48a5a316-e617-4526-8517-beda54295cbf`. The globe controls hold one position instead of shifting when zooming hides the pitch, and their glyphs are centred in the smaller 36px circles. Shape and Design now fit their panels with no scrolling at 390x844 and 320x568; they previously overflowed by 245px/111px and 350px/197px. The progress row, step headings and explanatory copy are much tighter, the size card, brushes and image controls sit on shorter rows, and short screens give the panel more height. `test:mobile-composition` walks the studio journey and fails if either stage starts scrolling again; that part needs the dev build's `geodesicQA` hook, so it reports as skipped against a deployed origin and must be run against `npm run dev`.

Previous frontend deployment (20 September 2026): `164d45ab-57e2-44ef-a860-eac9aefff196` from source commit `5f2a91c3`. Browse opens on the whole globe, its diameter spanning the viewport width, and pulls back half again further from there; measured live it opens at 390px of 390 and zooms out to 257px. Claim moved into the header beside owner sign-in, freeing the bottom edge for the proposition. The activity ticker no longer appears on the opening view and arrives at the bottom only once you zoom in. Control circles are 36px with a 44px touch target preserved, and the tour is back in the rail. This replaces the earlier overfilled framing. Verified live at 390x844 and 320x568 with globe occlusion at 0.8% and 6.5%.

Previous frontend deployment (20 September 2026): `c942b474-110b-48ee-8293-3803ef2bc505` from source commit `e0fbb9ad`, fixing the Browse-to-zoom transition. The phone composition was scoped `:not(.detail-view)`, so zooming in teleported Claim from the bottom sheet back to the topbar and reflowed the pitch into the desktop slab. Scoping it on `.creating` alone fixed that and exposed three collisions on the bottom edge, now also fixed: controls over Claim once the sheet slides away, the inspector over the controls, and the confirmation toast covering Claim entirely. A dismissed toast that kept swallowing clicks was a pre-existing bug and is fixed for every size. `test:mobile-composition` now guards the zoom transition. Verified live at 390x844 and 320x568.
