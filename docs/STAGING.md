...
Latest frontend deployment (14 September 2026): `5503b999-b6d0-47a9-b7ae-2bf7fa5af11d` from source commit `0a6d9ff3`, referencing immutable runtime release `6121418c93f84b0a7fd8b572f3f386de309edec9ba6bc2aefcc2d44e084374b2`. The narrow-screen canvas is now full-bleed; floating chrome is cleared with a camera view offset, the globe fills the width, and the existing hero/activity/action nodes form one bottom dock. The stable control rail retains 44px targets and the hero yields only after a real drag, so tapping a cell cannot reflow the camera between pointerdown and pointerup. Local acceptance covered 320x568, 390x844, 430x932 and 820x1180 plus desktop; live desktop/mobile Design -> Place -> Review -> Stripe mount/release/reload passed with zero browser errors and no payment submitted. Strict health/CDN checks passed with cache HITs after all 1,549 runtime objects were verified. Rendered local and live screenshots were inspected. Physical iOS/Android devices remain unverified; the existing renderer remains enabled (`VITE_ARTWORK_SNAPSHOTS=false`).
