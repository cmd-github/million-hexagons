...
Latest frontend deployment (12 September 2026): `ba928751-ab70-4ab0-b12e-f6f94e16c554`. Brighter dark-hemisphere lighting preserves globe shading and improves surface visibility. Local before/after and live desktop/mobile overview, rotation and zoom screenshots inspected with zero page errors; evidence: `artifacts/globe-lighting/`, reproduced by `node scripts/globe-lighting-qa.mjs` (set `MH_LIGHTING_URL` for staging). Staging build, all 1,549 runtime objects and live health/CDN checks passed. The existing renderer remains enabled (`VITE_ARTWORK_SNAPSHOTS=false`); no backend/compiler activation was performed. Physical devices remain unverified.
