...
Latest frontend deployment (15 September 2026): `ed183713-d0fb-405c-a8d5-a88f42210c36` from source commit `bd871d94`, retaining immutable runtime release `6121418c93f84b0a7fd8b572f3f386de309edec9ba6bc2aefcc2d44e084374b2`. Placement creation is now a two-step Design-on-globe -> Review flow: drawing onto blank connected hexagons adds them, Delete is the only removal action, four visible hexagonal brushes replace the dropdown, large minus/plus controls resize the Hexagons count, and Move globe/Move design keep location choices on Design. All 1,549 runtime objects and the post-deployment HTTPS/CDN health check passed with cache HITs. A fresh retry of the live desktop/mobile Design -> Review -> Stripe-mount journey passed with zero browser errors, no direct Firestore reads and no payment submitted; the first concurrent run timed out during globe startup, so startup reliability remains part of the open launch evidence.
