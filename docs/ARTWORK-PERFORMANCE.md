...
The circular grid patch has since been replaced by viewport-covering cached regions; see [current grid rendering and measurements](GRID-RENDERING.md). Full viewport readiness is now measured alongside frame times.

Earlier grid attempt (superseded): exact central geometry builds during zoom/flight, ahead of a wider patch. Current-view region requests promote queued downloads while preserving the four-request limit and deduplication. Cached geometry can build without waiting for an obsolete wide request. Outlines become visible at a lower pixel threshold and the initial fade is 150 ms instead of 800 ms. A controlled seven-wheel-step test measured first visible grid at 1.45 s desktop / 1.72 s emulated mobile, versus 7.21 s / 1.98 s before. These are single local runs with 10 Mbps / 80 ms network emulation and 4× mobile CPU slowdown, measured from zoom initiation; they are not full-page load or physical-device timings. Evidence: `artifacts/zoom-grid/{baseline,after}/`, `node scripts/zoom-grid-qa.mjs`. Exhaustive regional identity, boundary picking, request-priority and moving-camera tests pass. The artwork message after startup has been removed entirely.

Startup now hands over from the branded loader only after visible preview coverage is rendered. Coarse snapshot ancestors count as a preview, so sharp-tile downloads continue after reveal. Legacy placement loading waits for visible artwork, or the requested placement on a direct link; genuinely empty inventory is allowed. Later navigation loads silently without an artwork message. Eight-second slow-load copy and failure/thirty-second Retry prevent an unexplained endless animation. This improves the loading experience without changing the performance budgets. Reproduce with `node scripts/artwork-loading-qa.mjs` and `node scripts/snapshot-lifecycle-qa.mjs` after preparing the local snapshot fixture; screenshots are under `artifacts/artwork-loading/` and `artifacts/snapshot-lifecycle/`.
