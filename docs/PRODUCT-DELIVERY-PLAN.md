...
Implemented: close-range stationary hover with company, website and stable hex ID; company/hex search; click/tap inspection; separate outbound Visit; green exact hexagon counts; optional company name (60 characters) and description (160 characters) entered at Review; session preview dates and local outbound-link click counts. Sample branding remains explicitly labelled and never carries an invented purchase date. The HUD can be pinned, otherwise outside clicks dismiss it; Escape dismisses it even when pinned. Search also closes on outside clicks. Tour approach shows the targeted placement HUD. Home eases to the overview and resumes rotation, respecting reduced motion. Random discovery and View placement buttons were removed. The latest activity feed shows up to five actual session previews, expanded on desktop and collapsed on mobile. Shared #cell= links animate to their destination; session artwork is not durable or shared.
...
Outbound links (TODO 17): retain standard secure new-tab links. Browser preferences control focus; a website cannot reliably force a background tab/pop-under. The HUD provides the Ctrl/Cmd-click hint; native middle-click and browser context menus remain available.
