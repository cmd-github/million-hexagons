...
Implemented: close-range stationary hover with company, website and stable hex ID; company/hex search; click/tap inspection; separate outbound Visit; green exact hexagon counts; optional company name (60 characters) and description (160 characters) entered at Review; session preview dates and browser-persistent running outbound-link totals (localStorage, not shared analytics). Sample branding remains explicitly labelled and never carries an invented purchase date. The logo-led HUD uses small inline counts and an accessible icon pin switch. Same-owner hex clicks update only the hex number. The HUD can be pinned, otherwise outside clicks dismiss it; Escape dismisses it even when pinned. Search also closes on outside clicks. Tour approach shows the targeted placement HUD. Home eases to the overview and resumes rotation, respecting reduced motion. Random discovery and View placement buttons were removed. The latest activity feed shows up to five actual session previews plus three explicitly labelled illustrative examples, expanded on desktop and collapsed on mobile. Shared #cell= links animate to their destination; session artwork is not durable or shared.
