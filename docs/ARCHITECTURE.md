...
Artwork/camera polish: arbitrary image rotation fits the complete rotated bounding box before clipping; one 512px union-mask integral is reused during image framing and released on editor close. Rasterisation uses high-quality resampling without raising the 3072px artwork limit. Occupied-cell grid alpha is reduced in the existing shader. No extra render pass, tile resolution or cache capacity was added.
