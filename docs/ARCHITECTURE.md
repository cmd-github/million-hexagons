...
The first sandbox slice stores a bounded WebP artwork rendition in a separate `stagingPlacementVersions` document, keeping the potentially large encoded cell set below Firestore's per-document limit. Authenticated owner listing restores exact occupied cells, metadata and the rendition after reload. Owners may delete their own staging placements; the transactional staging delete releases inventory and marks the content version deleted. This is test-only behaviour. Full-resolution private sources and background compilation into immutable R2 releases remain the next publication slice.
