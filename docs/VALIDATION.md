...
Exploration TODO 14-26: `node scripts/exploration-qa.mjs` checks company search, sample labelling, local click counts, eased Home/rotation, tour HUD, hover IDs and outside dismissal on desktop/mobile. `globe-design-qa.mjs` additionally checks company metadata publication, feed entries and HUD pinning. Inspect `artifacts/exploration/` and `artifacts/globe-design/`. Counts/dates are session-only; this does not validate production analytics or paid claims.
