...
Activity/HUD/control changes: `node scripts/activity-hud-qa.mjs` checks desktop, mobile and tablet layout, directional rotation, restored Home orientation, altitude-scaled drag speed and available-cell prompts. Inspect `artifacts/activity-hud/`. Streaming priority changes also require `node --test scripts/artwork-stream.test.mjs`; example metrics do not validate real view measurement.
