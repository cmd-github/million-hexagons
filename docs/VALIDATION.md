...
Durable staging publication: run `npm.cmd run test:staging-publication`. The live disposable journey verifies private-source acceptance, durable owner reload, authoritative overlap rejection, background publication, immutable R2 artwork and metadata delivery, deletion/release and exact-cell reuse. It cleans up its staging claims and requires the ignored local QA secret; it does not validate payment, moderation or physical devices.
