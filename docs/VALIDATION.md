...
`npm run test:desktop-composition` holds the single-layout contract at 1440x900 and in 390x844 and 320x568 mobile browser contexts. It confirms that phones receive the readable 900px desktop layout viewport, that no mobile dock exists, and that the desktop header, hero, vertical control rail and fixed right-hand Design and Place panel retain their desktop geometry. Inspect `artifacts/desktop-composition/`. Emulated mobile contexts are not physical-device certification; confirm real browser viewport scaling and touch interaction on iOS and Android before launch.
