...
Implemented: close-range stationary hover with company, website and stable hex ID; company/hex search; click/tap inspection; separate outbound Visit; green exact hexagon counts; optional company name (60 characters) and description (160 characters) entered at Review; session preview dates and browser-persistent running outbound-link totals (localStorage, not shared analytics). Sample branding remains explicitly labelled and uses illustrative claim dates in presentation preview. The logo-led HUD uses small inline counts and an accessible icon pin switch. Same-owner clicks preserve the HUD without showing a clicked hex number. The HUD can be pinned, otherwise outside clicks dismiss it; Escape dismisses it even when pinned. Search also closes on outside clicks. Tour arrival shows the targeted placement HUD after framing the complete advert above the mobile card. Home eases to the overview and resumes rotation; reduced motion uses immediate navigation and keeps rotation paused. Random discovery and View placement buttons were removed. The latest activity feed shows up to five actual session previews plus four illustrative activity rows, expanded on desktop and collapsed on mobile. Shared #cell= links animate to their destination; session artwork is not durable or shared.
...
Home and the brand link restore initial orientation over 2.2 seconds; one button cycles left, pause, right, pause. Manual rotation scales down with altitude. The hard atmosphere ring and passive gesture icons are removed. Tile loading prioritises sharp visible requests within unchanged cache/concurrency limits; cold-network fallback blur remains possible.

Presentation layout: lower-corner cards retain a fixed activity viewport at overview and close zoom. Nearby opens above its disclosure as an overlay. The toolbar sits above the HUD, and the global metrics pill sits at top centre. Reduced motion switches metrics instantly. Background hexagons reuse the existing 950-point draw with one 32px texture.
